...
export default function CheckoutClient({ productId, titulo, emoji, loja, preco, cor, bump, aceitaCupom }: any) {
  ...
  async function pagar() {
    ...
    const res = await fetch('/api/checkout', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include',
      body: JSON.stringify({ productId, cupom: aplicado?.codigo || null, bump: bumpAceito, cpf: cpfDigits }),
    });
  ...
  return (
    <div className="card">
      ...
      <div style={{ padding: '4px 0 16px' }}>
        <label style={{ fontSize: 13, fontWeight: 700, display: 'block', marginBottom: 8 }}>CPF do pagador</label>
        <input value={cpf} onChange={(e) => setCpf(formatCpf(e.target.value))} placeholder="000.000.000-00" inputMode="numeric"
          style={{ width: '100%', height: 42, borderRadius: 10, border: '1px solid var(--border)', padding: '0 12px', fontSize: 14 }} />
        <small style={{ color: 'var(--muted)', fontSize: 12 }}>Necessário para gerar o Pix com segurança.</small>
      </div>
